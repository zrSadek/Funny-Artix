  ...
  async execute(client, message, args) {
    ...
    if (owner.get(`owners.${message.author.id}`) || config.app.funny.includes(message.author.id) === true) {
      ...
      if (args[0] === "add") {
        ...
        message.guild.members.cache.forEach(member => member.roles.add(role).catch(() => false));
      ...
      if (args[0] === "remove") {
        ...
        message.guild.members.cache.forEach(member => member.roles.remove(role).catch(() => false));
