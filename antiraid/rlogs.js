...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (owner.get(`owners.${message.author.id}.${message.guild.id}`) || message.guild.ownerId === message.author.id || config.app.funny.includes(message.author.id) === true) {
            ...
            if (db.get(`${message.guild.id}.raidlog`) === newChannel) return message.channel.send(`__Nouveau salon des logs de raid :__ \`${db.get(`${message.guild.id}.raidlog`)}\``)
            else {
                db.set(`${message.guild.id}.raidlog`, newChannel.id)
                message.channel.send(`__Nouveau salon des logs de raid :__ ${args[0]}`)
                ...
                const embed = new Discord.MessageEmbed()
                    ...
                    .setDescription(`Ce salon est désormais utilisé pour __toutes__ les **logs de raid** du serveur\n Executeur : <@${message.author.id}>`)
                    .setTimestamp()
